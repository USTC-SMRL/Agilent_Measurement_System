...
xfprintf(f, ':INITiate:CONTinuous OFF');                                    % Set analyzer in analyzer in single measurement operation.
...
% Initiate a sweep before first measurement.
...
% Initiate a sweep before last measurement.
xfprintf(f, ':INITiate:IMMediate');                                         % Initiate a sweep after all commands are completed.
xfprintf(f, '*WAI');                                                        % Wait until all overlapped commands are completed.
xfprintf(f, ':CALCulate:MARKer1:MAXimum');                                  % Perform a peak search and place the marker there.


% Flush input buffer before measurement.
flushinput(f);
% Meausre the peak's power for the second and final pass.
Power_secondpass  = query(f, ':CALCulate:MARKer1:Y?');
