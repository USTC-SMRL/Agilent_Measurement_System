...
% THIS SOFTWARE MAY ONLY BE USED IN CONJUNCTION WITH AGILENT INSTRUMENTS. 
    ...
    if (size(channelMapping, 1) > 2 && isfield(arbConfig, 'visaAddr2'))
        arb2 = arbConfig;
        arb2.visaAddr = arb2.visaAddr2;
        iqdownload_M8190A(arb2, fs, data, marker1, marker2, segmNum, keepOpen, channelMapping(3:4,:), sequence, run);
...
%                if (xfprintf(f, cmd, useM8192A) ~= 0)
        ...
        if (interpolationFactor ~= 1)
            ...
            for ch = find(channelMapping(:,1))'
                gen_arb_M8190A(arbConfig, f, ch, real(data), marker1, segmNum);
            end
            for ch = find(channelMapping(:,2))'
                gen_arb_M8190A(arbConfig, f, ch, imag(data), marker2, segmNum);
            end
        end
...
function doRun(f, arbConfig, useM8192A, channelMapping, run, keepOpen)
    if (useM8192A)
        ...
        if (run && ~strcmp(arbConfig.visaAddr, arbConfig.visaAddr2))
            try
                arbSync = loadArbConfig();
...
function gen_arb_M8190A(arbConfig, f, chan, data, marker, segm_num)
