        ...
        if (length(varargin) >= 2)
            switch varargin{2}
                ...
                case 'DigDemod'
                    ...
                    tr = vsaTraces.Item(2); tr.DataName = 'IQ Meas Time1';
                      tr.Format = Agilent.SA.Vsa.TraceFormatType.EyeI;
...
%                    return;
                    ...
                    newCol2 = interp1(ampCorr(:,1), ampCorr(:,2), x, 'pchip', 0);
                    if (size(ampCorr,2) >= 3)
                        newCol3 = interp1(ampCorr(:,1), ampCorr(:,3), x, 'pchip', 1);
                    else
